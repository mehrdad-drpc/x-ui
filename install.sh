...
config_after_install() {
    echo -e "${yellow}due to safety concerns，Install/After the update is complete, the port and account password must be changed forcibly${plain}"
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}detection x-ui version failed，may be beyond Github API limit，Please try again later，or specify manually x-ui version installation${plain}"
            exit 1
        fi
        echo -e "detected x-ui The latest version：${last_version}，start installation"
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-${arch}.tar.gz https://github.com/vaxilu/x-ui/releases/download/${last_version}/x-ui-linux-${arch}.tar.gz
        if [[ $? -ne 0 ]]; then
            echo -e "${red}download x-ui fail，Please make sure your server can download Github document${plain}"
            exit 1
        fi
    else
        last_version=$1
    ...
    config_after_install
    #echo -e "If fresh install，The default web port is ${green}54321${plain}，The default username and password are ${green}admin${plain}"
    ...
    #echo -e ""
    #echo -e "If update panel，then access the panel as you did before"
    #echo -e ""
    systemctl daemon-reload
    systemctl enable x-ui
    systemctl start x-ui
    echo -e "${green}x-ui v${last_version}${plain} The installation is complete，panel activated，"
    echo -e ""
